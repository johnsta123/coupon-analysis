...
### Data
1. toCoupon_GEQ5min — usually all 1s (every trip in this dataset is ≥5 min to the coupon location). A constant column has zero variance, so it carries no information and can't help distinguish acceptance vs. rejection. Safe to drop.
2. toCoupon_GEQ15min and toCoupon_GEQ25min — these do vary, and represent driving-distance thresholds. Not useless, but they overlap in information with each other (a coupon ≥25 min away is also ≥15 min away by definition) — so including both can introduce redundancy/multicollinearity if you're building a model.
3. direction_same — indicates whether the coupon venue is in the same direction as the driver's current travel direction. There's typically also a direction_opp column that's just the logical inverse (direction_same = 1 - direction_opp), so keeping both is redundant — one fully determines the other.
